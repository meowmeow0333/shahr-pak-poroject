...
"""
test_shahr_pak.py
testhaye vahed (Unit Tests) - pooshesh elzam faz 7 (hadeaghal 10 test khodkar).
ejra: python3 -m pytest test_shahr_pak.py -v
"""
...
def fresh_db(tmp_path, monkeypatch):
    """har test rooye yek paygah dadeye movaghat va tamiz ejra mishavad."""
...
# ---------------------------------------------------------------------------
# testhaye scenario 1: sbt gozaresh
# ---------------------------------------------------------------------------
...
def test_submit_report_success():
    ...
    report = service.submit_report(
        citizen_id=citizen_id, title="por shodan makhzan", description="makhzan por ast",
        location="khiaban Valiasr", lat=35.7, lng=51.4,
    )
...
def test_submitted_report_is_persisted_and_listed():
    service = ReportService()
    citizen_id = get_citizen_id()
    service.submit_report(citizen_id, "zobale dar khiaban", "tozih", "mahal test", 1.0, 1.0)
    reports = service.list_reports()
    assert any(r.location == "mahal test" for r in reports)


# ---------------------------------------------------------------------------
# testhaye scenario 2: taghyir vaziyat tavasot operator
# ---------------------------------------------------------------------------
...
def test_invalid_status_transition_raises():
    ...
    # nemitavan mostaghim az "ersal shode" be "takmil shode" parid
    with pytest.raises(ValueError):
...
def test_filter_reports_by_keyword():
    service = ReportService()
    citizen_id = get_citizen_id()
    service.submit_report(citizen_id, "case vijeh", "tozih khas", "meydan azadi", 1.0, 1.0)
    result = service.list_reports(keyword="vijeh")
    assert len(result) == 1
    assert result[0].title == "case vijeh"


# ---------------------------------------------------------------------------
# tst-hay service payesh makhzanha (Monitoring)
# ---------------------------------------------------------------------------
